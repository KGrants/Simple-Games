...
def game(home_team, away_team, year, g_type):
    ...
    home_points = int(
        100 * (1 + ((home_power[1] - away_power[2]) / 100)) * random.uniform(0.88, 1.18)
    )
    away_points = int(
        100 * (1 + ((away_power[1] - home_power[2]) / 100)) * random.uniform(0.85, 1.15)
    )
    ...
    cur.execute(
        """INSERT INTO Games 
                   ...
                   VALUES
                   (%s, '%s', %s, %s, %s, %s, %s)"""
        % (year, g_type, home_team, away_team, home_points, away_points, winner)
    )
    conn.commit()
    ...
    for key in h_player_points:
        cur.execute(
            """INSERT INTO Player_Score
                       VALUES
                       (%s, %s, %s, %s)"""
            % (key, home_team, game_id, h_player_points[key])
        )
    ...
    for key in a_player_points:
        cur.execute(
            """INSERT INTO Player_Score
                       VALUES
                       (%s, %s, %s, %s)"""
            % (key, away_team, game_id, a_player_points[key])
        )
    return
...
def point_distribution(points, team_id):
    ...
    player_points = {ids[i]: int(rat_weight * ratings[i]) for i in range(4)}
    player_points[ids[4]] = points - sum(player_points.values())
...
def playoffs(year):
    while True:
        ...
        for i in range(4):
            second_round.append(
                playoff_series(
                    playoff_tree[(0, 3, 1, 2)[i]][0], playoff_tree[(7, 4, 6, 5)[i]][0]
                )
            )
...
def playoff_series(team_a, team_b):
    ...
    while True:
        game(
            team_a if series[game_count] == "1" else team_b,
            team_b if series[game_count] == "1" else team_a,
            year,
            "P",
        )
        winner = cur.execute(
    ...
    print(
        f"{team_name} won the series ({wins_a if wins_a>wins_b else wins_b}:{wins_a if wins_a<wins_b else wins_b})"
    )
    return winner


def mvp_growth(year):
    """This function updates the offence and defence attributes of the player with the id of the Most Valuable Player (MVP) of the given year. It adds 5 to the offence and
    defence attributes of the MVP, but if the value exceeds 100, it sets the value to 100. This function is used to simulate the growth of the MVP's skills over time"""
    mvp = s.mvp(year)[2]
...
def draft(year):
    f.draft_team()
    for _ in range(10):
        f.create_player(limit=24, team_id=998)
